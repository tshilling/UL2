    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float3 normalLocal = normalize(fwidth(VertOffset));
...
////// Specular:
                ...
                float node_9872 = 0.0;
                float3 diffuseColor = (_Color.rgb+lerp((_FoamTexture_var.rgb+_Color_copy.rgb),float3(node_9872,node_9872,node_9872),saturate((sceneZ-partZ)/_FoamDistance))+((1.0-max(0,dot(normalDirection, viewDirection)))*_Trans_copy)); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 _FoamTexture_var = tex2D(_FoamTexture,TRANSFORM_TEX(i.uv0, _FoamTexture));
                float node_9872 = 0.0;
                float3 diffColor = (_Color.rgb+lerp((_FoamTexture_var.rgb+_Color_copy.rgb),float3(node_9872,node_9872,node_9872),saturate((sceneZ-partZ)/_FoamDistance))+((1.0-max(0,dot(normalDirection, viewDirection)))*_Trans_copy));
                float specularMonochrome;
                float3 specColor;
