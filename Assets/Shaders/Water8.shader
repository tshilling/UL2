...
Shader "Shader Forge/Water8" {
    Properties {
        ...
        _Color_copy ("Color_copy", Color) = (0,0,0.1,1)
...
////// Specular:
                ...
                float node_530 = 0.0;
                float3 diffuseColor = (pow(1.0-max(0,dot(node_5365, viewDirection)),_FresnelPower)+_Color.rgb+lerp((_FoamTexture_var.rgb+_Color_copy.rgb),float3(node_530,node_530,node_530),saturate((sceneZ-partZ)/_FoamDistance))); // Need this for specular when using metallic
...
/// Final Color:
                ...
                fixed4 finalRGBA = fixed4(finalColor,saturate((sceneZ-partZ)/_EdgeDistance));
...
/// Final Color:
                float3 finalColor = diffuse + specular;
                fixed4 finalRGBA = fixed4(finalColor * saturate((sceneZ-partZ)/_EdgeDistance),0);
                UNITY_APPLY_FOG(i.fogCoord, finalRGBA);
                return finalRGBA;
            }
            ENDCG
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : SV_Target {
                ...
                float4 _FoamTexture_var = tex2D(_FoamTexture,TRANSFORM_TEX(i.uv0, _FoamTexture));
                float node_530 = 0.0;
                float3 diffColor = (pow(1.0-max(0,dot(node_5365, viewDirection)),_FresnelPower)+_Color.rgb+lerp((_FoamTexture_var.rgb+_Color_copy.rgb),float3(node_530,node_530,node_530),saturate((sceneZ-partZ)/_FoamDistance)));
                float specularMonochrome;
                float3 specColor;
